...
@section('content')
    ...
    <div id="report-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="bg-white w-11/12 max-w-4xl rounded-lg shadow-lg">
            ...
            <div class="flex justify-end px-6 py-4 border-t">
                <button id="print-report" class="text-white bg-blue-500 px-4 py-2 rounded hover:bg-blue-600">
                    Print
                </button>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const totalLecturersElem = document.getElementById('total-lecturers');
            const totalQuotaElem = document.getElementById('total-quota');
            const availableQuotaElem = document.getElementById('available-quota');
            ...
            // 1. Filter button event
            filterSemesterButton.addEventListener('click', () => {
                currentSemester = document.getElementById('semester').value;
                fetchData(currentSemester);
            });

            // 2. Fetch Data (GET /filter-lecturer-quota?semester=...)
            function fetchData(semester = '') {
                // Show a quick "loading" row
                tableBody.innerHTML = `
                    <tr>
                        <td colspan="6" class="text-center text-gray-500 py-4">Loading...</td>
                    </tr>
                `;
                ...
                fetch(`/filter-lecturer-quota?semester=${semester}`, {
                    ...
                    .then(data => {
                        ...
                        if (!data.success) {
                            tableBody.innerHTML = `
                            <tr>
                                <td colspan="6" class="text-center text-red-500 py-4">Failed to fetch data.</td>
                            </tr>
                        `;
                            return;
                        }
                        ...
                        if (data.stats) {
                            totalLecturersElem.textContent = data.stats.totalLecturers || 0;
                            totalQuotaElem.textContent = data.stats.totalQuota || 0;
                            availableQuotaElem.textContent = data.stats.availableQuota || 0;
                        }
            ...
            // 3. Generate Report (Open Modal)
            generateReportBtn.addEventListener('click', async () => {
                try {
                    // Same fetch with the currently chosen semester
                    const response = await fetch(`/filter-lecturer-quota?semester=${currentSemester}`, {
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest'
                        },
                    });
                        ...
                        lecturers.forEach((quota, index) => {
                            const row = `
                                <tr>
                                    ...
                                    <td class="border px-4 py-2">${quota.lecturer.program?.name ?? 'N/A'}</td>
            ...
            // 6. Initial Load (if you want some data shown immediately)
            fetchData();
        });
    </script>
@endsection
